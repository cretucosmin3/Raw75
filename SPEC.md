...
Slider dragging only re-runs what changed. Moving exposure does not re-run denoise if denoise inputs didn’t change. We still keep this to a handful of passes, not a darktable module graph.
...
That is the Lightroom idea. We are not storing darktable-style scene-referred graphs.
...
- No darktable pixelpipe (dozens of modules, full-buffer, hitchy).
